...
export default function useMeterData(fromDate, toDate) {
  const [locations, setLocations] = useState([]);
  const [selectedArea, setSelectedArea] = useState(null);
  const [filteredLocations, setFilteredLocations] = useState([]);
  const [selectedParam, setSelectedParam] = useState(null);
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/meters', {
      ...
      .catch(console.error);
  }, [heatmapAgg, selectedParam, filteredLocations, fromDate, toDate]);

  // Fetch meter parameter data
  useEffect(() => {
    if (!selectedParam || filteredLocations.length === 0 || aggregation.length === 0) {
      setMeterParamData({});
      return;
    ...
    fetch('http://localhost:5000/api/meters', {
      ...
      .catch(console.error);
  }, [selectedParam, filteredLocations, aggregation, fromDate, toDate]);
